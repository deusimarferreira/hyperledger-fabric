...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.villalabs.co | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.villalabs.co
    docker cp -a orderer.villalabs.co:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.villalabs.co
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.villalabs.co

    for PEER in peer0.org1.villalabs.co peer1.org1.villalabs.co peer0.org2.villalabs.co peer1.org2.villalabs.co; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.villalabs.co/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.villalabs.co/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.villalabs.co`` & ``peer0.org2.villalabs.co``).  Secondly, we point to
